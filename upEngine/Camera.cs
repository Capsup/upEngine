...
namespace upEngine
{
    public class Camera
    {
        ...
        {
            double DeltaX = ( (window.X + window.Width / 2 ) - System.Windows.Forms.Cursor.Position.X ) * 0.001f;
            double DeltaY = ( (window.Y + window.Height / 2 ) - System.Windows.Forms.Cursor.Position.Y ) * 0.001f;
            System.Windows.Forms.Cursor.Position = new Point( window.X + window.Width / 2, window.Y + window.Height / 2 );

            if( DeltaX != 0f )
            {
                TargetPos -= Origin;
                TargetPos = Vector3.Transform( TargetPos, Quaternion.FromAxisAngle( new Vector3( 0f, 1f, 0f ), (float)DeltaX ) );
            ...
            if( DeltaY != 0f )
            {
                ...
                double Angle = MathHelper.RadiansToDegrees( Math.Acos( Vector3d.Dot( new Vector3d( DirForward.X, DirForward.Y, DirForward.Z ), new Vector3d( DirTargetToOrigin.X, DirTargetToOrigin.Y, DirTargetToOrigin.Z ) ) ) );

                if( DirTargetToOrigin.Y < 0 )
                    Angle *= -1f;

                Angle += DeltaY;
                if( Angle > -89.80f && Angle < 89.80f )
                {
                    TargetPos -= Origin;
                    TargetPos = Vector3.Transform( TargetPos, Quaternion.FromAxisAngle( Vector3.Normalize( Vector3.Cross( TargetPos - Origin, Up ) ), (float)DeltaY ) );
                    //TargetPos = Vector3.TransformVector( TargetPos, Matrix4.CreateFromAxisAngle( Vector3.Normalize( Vector3.Cross( TargetPos - Origin, Up ) ), (float)DeltaY ) );
                    TargetPos += Origin;
                }
            }
